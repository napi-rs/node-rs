...
switch (platform) {
  ...
  case 'linux':
    switch (arch) {
      ...
      case 'riscv64':
        if (isMusl()) {
          localFileExisted = existsSync(join(__dirname, 'xxhash.linux-riscv64-musl.node'))
          try {
            if (localFileExisted) {
          ...
          localFileExisted = existsSync(join(__dirname, 'xxhash.linux-riscv64-gnu.node'))
          try {
            if (localFileExisted) {
...
if (!nativeBinding || process.env.NAPI_RS_FORCE_WASI) {
  try {
    localFileExisted =
      existsSync(join(__dirname, 'xxhash.wasm32-wasi.wasm')) && existsSync(join(__dirname, 'xxhash.wasi.cjs'))
    if (localFileExisted) {
