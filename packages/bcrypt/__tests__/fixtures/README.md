...
The test suite additionally uses the pinned `bcrypt-previous` npm alias (1.10.9) to verify newly created hashes and checks independent known answers with bcryptjs. Argon2's separate `historical-hashes.json` contains 48 PHC strings from published 1.8.3, 2.0.0, 2.0.2, and 2.2.1, spanning algorithms, versions, and external secrets. In 1.8.3 encoded hashing ignored the requested salt; those stored strings must still verify unchanged.
