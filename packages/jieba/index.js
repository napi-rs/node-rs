...
switch (platform) {
  ...
  case 'linux':
    switch (arch) {
      ...
      case 'riscv64':
        if (isMusl()) {
          localFileExisted = existsSync(join(__dirname, 'jieba.linux-riscv64-musl.node'))
          try {
            if (localFileExisted) {
          ...
          localFileExisted = existsSync(join(__dirname, 'jieba.linux-riscv64-gnu.node'))
          try {
            if (localFileExisted) {
...
if (!nativeBinding || process.env.NAPI_RS_FORCE_WASI) {
  try {
    localFileExisted =
      existsSync(join(__dirname, 'jieba.wasm32-wasi.wasm')) && existsSync(join(__dirname, 'jieba.wasi.cjs'))
    if (localFileExisted) {
